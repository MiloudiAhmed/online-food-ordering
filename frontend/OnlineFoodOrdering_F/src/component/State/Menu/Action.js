...
export const createMenuItem = ({ menuItemData, jwt }) => {
  return async (dispatch) => {
    dispatch({ type: CREATE_MENU_ITEM_REQUEST });
    try {
      const { data } = await api.post("/api/admin/food", menuItemData, {
        headers: {
...
// Récupérer les items de menu par restaurant
...
// Rechercher des items de menu
export const searchMenuItem = ({ query, jwt }) => {
  return async (dispatch) => {
    dispatch({ type: SEARCH_MENU_ITEM_REQUEST });
    try {
      const { data } = await api.get(`/api/food/search?name=${query}`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
      dispatch({ type: SEARCH_MENU_ITEM_SUCCESS, payload: data });
      console.log("Menu items search successful", data);
    } catch (error) {
...
// Mettre à jour la disponibilité d'un item de menu
